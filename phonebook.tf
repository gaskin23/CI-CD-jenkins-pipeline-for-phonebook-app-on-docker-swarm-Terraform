...
resource "aws_instance" "docker-machine-managers" {
  ...
  tags = {
    Name = "gaskin-Docker-Swarm-Manager-${count.index + 1}"
...
resource "aws_instance" "docker-machine-workers" {
  ...
  tags = {
    Name = "gaskin-Docker-Swarm-Worker-${count.index + 1}"
...
resource "aws_security_group" "tf-docker-sec-gr" {
  name = "gaskin-docker-swarm-sec-gr"
  tags = {
    Name = "swarm-sec-gr"
  }
  dynamic "ingress" {
